...
export const fetchAllCategories = createAsyncThunk('categories/fetchAllCategories', async () => {
  const response = await axios.get('https://nouman-cart.onrender.com/admin-category/all');
...
export const fetchSingleCategory = createAsyncThunk('categories/fetchSingleCategory', async (categoryId) => {
  const response = await axios.get(`https://nouman-cart.onrender.com/admin-category/${categoryId}`);
 console.log('response23',response)
  return response.data;
});

export const deleteCategory = createAsyncThunk('categories/deleteCategory', async (categoryId) => {
  await axios.delete(`https://nouman-cart.onrender.com/admin-category/delete/${categoryId}`);
  return categoryId;
});

export const createCategory = createAsyncThunk('categories/createCategory', async (categoryData) => {
  const response = await axios.post('https://nouman-cart.onrender.com/admin-category/add-category', categoryData);
  return response.data;
});

export const updateCategory = createAsyncThunk('categories/updateCategory', async ({ categoryId, formData }) => {
  const response = await axios.put(`https://nouman-cart.onrender.com/admin-category/update/${categoryId}`, formData);
  return response.data;
});
